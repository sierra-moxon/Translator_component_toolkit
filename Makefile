...
# Run spell checking
spell:
	uv run codespell --skip=metaData,notebooks,htmlcov,PathFinder_results,*.json --ignore-words-list=EHR

# Run spell checking interactively (allows fixing)
spell-fix:
	uv run codespell --interactive 3 --skip=metaData,notebooks,htmlcov,PathFinder_results,*.json --ignore-words-list=EHR
